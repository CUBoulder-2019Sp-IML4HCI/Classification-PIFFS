...
void showMessage(int i) { //TODO: change wave.setFrequency here based on i
    switch(i) { 
        case 1: 
            ...
            currentHue = (int)generateColor(5);
            currentTextHue = (int)generateColor((99));
            wave.setFrequency((float)(261 * Math.pow(1.059, i*2)));
            wave.setAmplitude(0.0); 
            break; 
        case 2: 
            currentMessage = "Need a coffee?";
            currentHue = (int)generateColor(5);
            currentTextHue = (int)generateColor((5+1));
            wave.setFrequency((float)(261 * Math.pow(1.059, i*2)));
            wave.setAmplitude(0.05);  
            break; 
        case 3: 
            currentMessage ="WAKE UP!";
            currentHue = (int)generateColor(1);
            currentTextHue = (int)generateColor((1+1));
            wave.setFrequency((float)(261 * Math.pow(1.059, i*2)));
